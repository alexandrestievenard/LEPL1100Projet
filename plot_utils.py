# plot_utils.py
# module de visualisation du projet, amélioré le rendu par rapport à la version originelle
#-> tracé plus fluide et plus robuste
import numpy as np
...
def plot_fe_solution_2d(elemNodeTags, nodeCoords, nodeTags, U, tag_to_dof, show_mesh=False, ax=None, vmin=0.0, vmax=50.0, cmap='plasma'):
    """
    ...
    contour = ax.tricontourf(x, y, triangles, U_plot, levels=np.linspace(vmin, vmax, 256), cmap=cmap, vmin=vmin, vmax=vmax, extend='max'   # valeurs > vmax → couleur maximale (pour le bocage K=80)
    )
